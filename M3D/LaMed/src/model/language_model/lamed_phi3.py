    ...
    def generate(
        ...
        **kwargs,
        ...
        if seg_enable:
            ...
            step_0_hidden_states,  # Tuple of [layer_0, layer_1, ..., layer_n]
            step_1_hidden_states,
            ...
            step_T_hidden_states
        ]
        step = (
    layer_0_output,  # Tensor shape: [batch_size, 1, hidden_dim]
    layer_1_output,
    ...
    layer_n_output   # ← step[-1] gives last layer
)
            ...
            for i in range(len(seg_token_mask)):
                # seg_token_mask: a tensor of shape [batch_size, seq_len],
                #  where True marks the position of a segmentation token (seg_token_id)
                # loop over each sample in batch
                # meaning of seg_token_mask[i]
                if torch.sum(seg_token_mask[i]) == 1:
                    # seg_token_mask[i]: a boolean mask of shape [seq_len].
                            # las _hidden_state [batch_size, seq_len, hidden_dim]
                            # last_hidden_state[i]: shape [seq_len, hidden_dim]

                            # last_hidden_state[i][seg_token_mask[i]]: returns a tensor of shape [1, hidden_dim]
                            # → the hidden state of the segmentation token.
                            # seg_token_mask[i] [seq_len] This is a Boolean mask, where True marks positions of the segmentation token in the sequence.
                    seg_token = last_hidden_state[i][seg_token_mask[i]]
                    # seg_token: [1, hidden_dim]
                    seg_prompt = self.get_model().seg_projector(seg_token)
                elif torch.sum(seg_token_mask[i]) > 1:
                    seg_tokens = last_hidden_state[i][seg_token_mask[i]]
                    seg_token = torch.mean(seg_tokens, dim=0, keepdim=True)
