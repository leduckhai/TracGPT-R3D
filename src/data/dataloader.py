...
def load_data( train_val_dir ="/root/VLMTrac/chunks/train/data", image_path="/root/VLMTrac/2d_data/train/image",bbox_only=False,dataset="trac"):
    if dataset=="trac":
        dataset=TracDataset
    ...
    train_set=dataset(data_paths=train_paths, image_path=image_path, mode="train", n_sample=train_sample,bbox_only=bbox_only)
    val_set=dataset(data_paths=val_paths,image_path=image_path, mode="val", n_sample=val_sample)
    test_set=dataset(data_paths=test_paths,image_path=image_path, mode="test", n_sample=test_sample)
    return train_set, val_set, test_set
if __name__ == "__main__":
    import os
    
    # train_set, val_set, test_set = load_data()
    train_set, val_set, test_set = load_data(train_val_dir="/root/TracGPT-R3D/pseudo_3d/32_overlap_slices/0691cd9f-8dad-4005-811d-34fb610d4f88/train/data",dataset="trac_white")
