...
class TracDatasetVerify(Dataset):
    def __init__(self, data_paths, image_path, mode="train", n_sample=-1, image_shape=[32, 256, 256], dataset_config=None, balance_A4=True,is_transform=False):
    ...
    def _load_image(self, patient_id, slice_order,bbox_2d_lists=None):
        # print("bbox_2d_lists:", bbox_2d_lists)
        image_paths = [os.path.join(self.img_dir, patient_id, f"{s}.pkl") for s in slice_order]
        return convert_list_slice_paths_with_bbox_to_3d(image_paths, bbox_2d_lists) if bbox_2d_lists else convert_list_slice_paths_to_3d(image_paths)
    ...
    def __getitem__(self, idx):
        ...
        image = self._load_image(data_point["Patient ID"], data_point["slice order"], data_point.get("A1", None))
        answer = ""
        if self.mode !="test":
            pretext="The diagnosis is"
            answer = data_point["A4"].lower().replace("-", " ")
            answer = f"{pretext} {answer}."
