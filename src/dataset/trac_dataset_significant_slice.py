...
import pandas as pd
class TracSignificantDataset(Dataset):
    def __init__(self, data_paths, image_path,significant_slice_path, mode="train", n_sample=-1, image_shape=[32, 256, 256],  balance_A4=True,is_transform=True):
        ...
        # self.significant_slice_path=significant_slice_path
        self.df=pd.read_csv(significant_slice_path)
        random.seed(68)   
        all_data = []
        print("mode", mode)
    ...
    def __getitem__(self, idx):
        ...
        significant_slices=self.df[self.df["Patient ID"] == data_point["Patient ID"]][["Slide"]].to_list()
        significant_indices=[data_point["slice order"].index(s) for s in significant_slices if s in data_point["slice order"]]
        limit=3
        significant_indices=significant_indices[:limit]
        answer = ""
        if self.mode !="test":
